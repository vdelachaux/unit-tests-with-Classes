...
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isText; "Wrong type")
ASSERT:C1129($test.error="messages: 'Text (2)' TYPE MISMATCH - Text instead of Null")

$test:=$ut.test("Strict Text").expect(getText).noReport().strict().isEqualTo(getText(1)).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isText; "Wrong type")
ASSERT:C1129($test.error="messages: 'Strict Text' gives 'Hello World' when 'hello world' was expected")

$test:=$ut.test("Null").expect(Null:C1517).noReport().isEqualTo(getText).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isNull; "Wrong type")
ASSERT:C1129($test.error="messages: 'Null' TYPE MISMATCH - Null instead of Text")
...
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isNull; "Wrong type")
ASSERT:C1129($test.error="messages: 'Null' TYPE MISMATCH - Null instead of Object")
...
ASSERT:C1129($test.error="messages: 'Object' TYPE MISMATCH - Object instead of Null")

$test:=$ut.test("Object").expect(getObject).noReport().isEqualTo(getObject(3)).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Object' Property '.foo' type mismatch. ACTUAL (Object) EXPECTED (Text)")

$test:=$ut.test("Object (2)").expect(getObject).noReport().isEqualTo(New object:C1471).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Object (2)' Property number mismatch ACTUAL (0) EXPECTED (1)")

$test:=$ut.test("Object (3)").expect(getObject).noReport().strict().isEqualTo(getObject(1)).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Object (3)' Property '.foo' mismatch ACTUAL (\"BAR\") EXPECTED (\"bar\")")
...
ASSERT:C1129($test.error="messages: 'Collection' Collection properties doesn't have expected length ACTUAL (2) EXPECTED (0)")

$test:=$ut.test("Collection (2)").expect(New collection:C1472).noReport().isEqualTo(Null:C1517).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isCollection; "Wrong type")
ASSERT:C1129($test.error="messages: 'Collection (2)' TYPE MISMATCH - Collection instead of Null")

$test:=$ut.test("Collection (3)").expect(getCollection).noReport().strict().isEqualTo(getCollection(1)).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isCollection; "Wrong type")
ASSERT:C1129($test.error="messages: 'Collection (3)' Property '[1]' mismatch ACTUAL (\"bar\") EXPECTED (\"BAR\")")

$test:=$ut.test("Collection (4)").expect(getCollection(2)).noReport().strict().isEqualTo(getCollection(3)).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isCollection; "Wrong type")
ASSERT:C1129($test.error="messages: 'Collection (4)' Collection properties '[1]' doesn't have expected length ACTUAL (2) EXPECTED (3)")
...
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isPointer; "Wrong type")
ASSERT:C1129($test.error="messages: 'Pointer (2)' TYPE MISMATCH - Pointer instead of Nil pointer")

$test:=$ut.test("Pointer (3)").expect(getPointer).noReport().isEqualTo(New object:C1471).latest
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isPointer; "Wrong type")
ASSERT:C1129($test.error="messages: 'Pointer (3)' TYPE MISMATCH - Pointer instead of Object")
...
ASSERT:C1129($test.failed; "Should failed")
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Object': isTrue() cannot be applied to the type Object")
...
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Object (2)': isFalse() cannot be applied to the type Object")

$test:=$ut.test("Not Null").expect("").noReport().isNull().latest
ASSERT:C1129($test.failed; "Should fail")
ASSERT:C1129($test.isText; "Wrong type")
ASSERT:C1129($test.error="messages: 'Not Null' as returned an non null value")

$test:=$ut.test("Not Null (2)").expect(getText).noReport().isNull().latest
ASSERT:C1129($test.failed; "Should fail")
ASSERT:C1129($test.isText; "Wrong type")
ASSERT:C1129($test.error="messages: 'Not Null (2)' as returned an non null value")

$test:=$ut.test("Not Null (3)").expect(getObject).noReport().isNull().latest
ASSERT:C1129($test.isObject; "Wrong type")
ASSERT:C1129($test.error="messages: 'Not Null (3)' as returned an non null value")

$test:=$ut.test("Null").expect(Null:C1517).noReport().isNotNull().latest
ASSERT:C1129($test.failed; "Should fail")
ASSERT:C1129($test.isNull; "Wrong type")
ASSERT:C1129($test.error="messages: 'Null' as returned an non null value")
...
ASSERT:C1129($test.isInteger; "Wrong type")
ASSERT:C1129($test.error="messages: 'is empty Numeric: isEmpty/isNotEmpty can't be applied to the type Real")

$test:=$ut.test("is empty Boolean").expect(False:C215).noReport().isEmpty().latest
ASSERT:C1129($test.failed; "Should fail")
ASSERT:C1129($test.isBoolean; "Wrong type")
ASSERT:C1129($test.error="messages: 'is empty Boolean: isEmpty/isNotEmpty can't be applied to the type Boolean")
...
ASSERT:C1129($test.failed; "Should fail")
ASSERT:C1129($test.isNumeric; "Wrong type")
ASSERT:C1129($test.error="messages: 'invalid target for toLength': toLength() can't be applied to the type Object")
